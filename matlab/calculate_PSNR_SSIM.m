...
% GT and SR folder
folder_GT = '../../data/Set5';
folder_SR  = '../experiments/sum stacksr wlr=1e-2 2x nonet/preds/Set5';
scale = 2;
...
filepaths = [filepaths; dir(fullfile(folder_GT, '*.jpg'))];
filepaths = [filepaths; dir(fullfile(folder_GT, '*.bmp'))];

testpaths = dir(fullfile(folder_SR, '*.png'));
testpaths = [testpaths; dir(fullfile(folder_SR, '*.jpg'))];
testpaths = [testpaths; dir(fullfile(folder_SR, '*.bmp'))];
...
%     im_SR  = imread(fullfile(folder_SR, [im_name(1:end-4), suffix, '.png']));
    im_SR  = imread(fullfile(folder_SR, testpaths(idx_im).name));
